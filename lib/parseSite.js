...
export class siteParser {
	...
	async getAssets(postBody) {
		...
		if (images) {
			for (const image of images) {
				...
				console.log(`Working on ${image.getAttribute("src")}...`);
				if (this.assetList.has(url)) image.setAttribute("src", this.assetList.get(url).fileName);
					...
					console.log(`Fetching asset at "${url} to ${assetFileName}...`);
					if (url.toLowerCase().startsWith("http:") || url.toLowerCase().startsWith("https:")) {
						if (await this.downloadFileTo(url, assetFileName)) {
						...
						if (this.copyFileTo(url, assetFileName)) {
							image.setAttribute("src", "assets/" + assetFileName);
							this.assetList.set(url, {
								URL: url,
								fileName: assetFileName
	...
	async downloadFileTo(url, filename) {
		...
		const foundMirror = this.findFileMirror(url);
		if (foundMirror) {
			fs.copyFileSync(foundMirror, location);
			return true;
		} else {
			let downloadRequest;
			try {
				...
				if (url.startsWith("http:")) {
					console.log(`Couldn't fetch with http, trying https`);
					try {
						downloadRequest = await fetch("https://" + url.substring(7));
					} catch (e) {
			...
			try {
				...
				Readable.fromWeb(downloadRequest.body).pipe(fileStream);
			} catch (e) {
