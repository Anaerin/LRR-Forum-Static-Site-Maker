...
export default class siteParser {	
	...
	async recordMissingAssets() {
		const missing = [];
		this.missingAssets.forEach((value, key) => {
			missing.push({URL: key, fileName: value});
		});
		await this.db.models.MissingAsset.bulkCreate(missing, {
			fields: ["URL", "fileName"],
	...
	async parseSite() {
		...
		const assets = await this.db.models.MissingAsset.findAll({raw: true});
	...
	async parseForumPage(file) {
		...
		console.log(`Created topics for ${file}, Currently ${getIdlessUserCount()} ID-less users and ${this.missingAssets.size} missing assets...`);
	}
	parseTopicLine(line) {
		let isSticky = false;
	...
	async parseTopicData(text, file) {
		...
		console.log(`Written ${file} to DB, Currently ${getIdlessUserCount()} ID-less users... and ${this.missingAssets.size} missing assets...`);		
	}
	...
	async updateAssetDB() {
		try {
			...
			if (newAssets.length > 0) {
				await this.db.models.MissingAsset.bulkCreate(newAssets, {
		...
		if (images) {
			for (const image of images) {
				...
				if (this.assetList.has(url)) image.setAttribute("src", this.assetList.get(url).fileName);
					...
					this.missingAssets.set(url, assetFileName);
				}
			}
		}
	}
