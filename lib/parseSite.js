...
export default class siteParser {	
	...
	async updateAssetDB() {
		try {
			...
			console.error(`Couldn't save assets to DB: ${e}`);
		}
		finally {
			this.assetList.clear();
			const assets = await this.db.models.Asset.findAll({raw: true});
	...
	*/
	async downloadFileTo(url, filename) {
		...
		try {
			...
			await new Promise((resolve, reject) => {
				const downloadStream = Readable.fromWeb(downloadRequest.body);
				downloadStream.on("error", (err) => {
					console.log(`Failed to stream to file: ${err}`);
					reject();
				});
				finished(downloadStream.pipe(fileStream), { cleanup: true }).then(resolve);
			});
		} catch (e) {
	...
	copyFileTo(url, filename) {
		...
		try {
			...
			console.error(`Unable to copy file from ${filePath} to ${location}: ${e}`);
			return;
		}
		if (!fs.existsSync(location)) {
			...
			console.log(`Copied ${filePath} to ${location}.`);
			return true;
		}
	}
