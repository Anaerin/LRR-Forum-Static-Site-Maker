...
export function writeFile(filename, contents) {
	...
	try {
		fs.writeFileSync(resolvedPath, contents, { flush: true, encoding: "utf8" });
	} catch(e) {
		console.log(`Util couldn't write to file, filename: ${resolvedPath}, Data: "${contents}": ${e}`);
	}
}
...
export async function copyStatic() {
	...
	fs.cpSync("./static/",config.outputFolder, { recursive: true, force: true, errorOnExist: false });
}