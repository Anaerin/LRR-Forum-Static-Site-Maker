import { createLink, deleteFile, moveFile, writeFile } from "./utils.js";
import templates from "../templates/index.js";
import config from "../config.js";
import { parse } from "node-html-parser";
import progress from "./progress.js";
import { index, putDocument, writeIndexFile, startImport } from "./search.js";
...
export default class siteExporter {
	...
	async renderForumPages(finishedFindingMissingAssets = false) {
		...
		const indexObj = {url: "index.htm"};
		["index.php","login.php","medals.php","memberlist.php","modcp.php","posting.php","privmsg.php","profile.php","report.php","search.php","ucp.php"].forEach((entry) => {
			this.phpRules.set(entry, [indexObj]);
		});
		...
		progress.completeTask("writeForums");
	}
	...
	async renderTopicPages(forumId) {
		...
		progress.startTask("writeTopics");
		const topics = await this.db.query(`
			WITH 
				LatestPostEntries AS (
					SELECT Posts.id, Users.name, Posts.datePosted, Posts.TopicId
					FROM Posts
				...
				PostCounts AS (
					SELECT COUNT(Posts.id) AS PostCount, Posts.TopicId
					FROM Topics
			...
			TopicUsers.name AS TopicUserName, LatestPost.id AS PostID, LatestPost.name AS LatestPostUserName, LatestPost.datePosted, PostCount.PostCount AS PostCount
			FROM Topics
			LEFT JOIN LatestPostEntries AS LatestPost ON LatestPost.TopicId = Topics.id
			LEFT JOIN PostCounts AS PostCount ON Topics.id = PostCount.TopicId
		...
		for (let page = 0; page <= pages; page++) {
			progress.updateTask("writeTopics", page, pages);
			...
			if (page > 0) {
				this.htaccessRules.push({
					...
					rules: [
						"viewforum\.php",
						`[\?&]f=${forumId}`,
						`[\?&]start=${page * config.topicsPerPage}`
					]
				});
				newRule.f=forumId;
				newRule.start = page * config.topicsPerPage;
			}
			else {
				this.htaccessRules.push({
					url: pageLink,
			...
			newRule.url = pageLink;
			let rules = [];
			if (this.phpRules.has("viewforum.php")) rules = this.phpRules.get("viewforum.php");
			rules.push(newRule)
			this.phpRules.set("viewforum.php", rules);
			console.log(`writing ${pageLink}...`);
			this.writeFile(pageLink, pageContents);
			progress.completeTask("writeTopics");
			topicPageContents = "";
		}
	}
	...
	async exportForums() {
		...
		for (const forumId of this.forumsWithTopics) {
			...
			counter = 0;
			progress.startTask("writePosts")
			await startImport();
			for (const topic of postTopics) {
				counter++;
				progress.updateTask("writePosts", counter, postTopics.length);
				...
				if (postCount > 0) {
					while (start < postCount) {
						...
						for (const post of posts) {
							...
							for (const image of postBody.querySelectorAll("img")) {
								...
								if (this.assets.has(URL)) {
									image.setAttribute("originalsrc", image.getAttribute("src"));
									image.setAttribute("src", "/assets/" + this.assets.get(URL));
								}
							}
							postDetails.body = postBody.toString();
							postLines.push(templates.postline(postDetails));
							searchPosts.push({
								page: pageLink + "#p" + post.id,
								userName: postDetails.user.name,
								subject: post.subject,
								datePosted: post.datePosted,
								body: postBody.toString(),
								forum: this.foraById.get(forumId).name,
								forumLink: createLink("forum", {f: forumId}),
								thread: topic.name,
								threadLink: threadLink
							});
						}
						await putDocument(searchPosts);
						...
						let phpRule = [];
						if (this.phpRules.has("viewtopic.php")) phpRule = this.phpRules.get("viewtopic.php");
						phpRule.push({
							p: postHTAccess,
							url: pageLink
						});
						...
						this.phpRules.set("viewtopic.php", phpRule);
			...
			progress.completeTask("writePosts");
		};
		console.log(`Writing htaccess.txt file`);
		this.writeFile(".htaccess", this.renderHTAccess());
		this.renderPHPBouncers();
	...
	renderHTAccess() {
		const HTAccessContents = this.htaccessRules.reduceRight((acc, rule) => {
			const url = rule.url;
			const ruleEntries = rule.rules.reduce((rules, ruleLine) => {
				if (Array.isArray(ruleLine)) {
					const orRuleArr = ruleLine.map((orLine) => `RewriteCond ${orLine}`);
					return rules + orRuleArr.join(' [or]\n') + "\n";
				} else return rules + `RewriteCond ${ruleLine}\n`;
			},"")
			return `${acc}

${ruleEntries}RewriteRule ${url} [R=301,L]
`;
		},"");
...
${HTAccessContents}`;
	...
	renderPHPBouncers() {
		this.phpRules.forEach((value, key) => {
			let out = "";
			value.forEach((entry) => {
				let url = "", ifs=0;
				for (const obj of Object.getOwnPropertyNames(entry)) {
					if (obj == "url") url = entry[obj];
					else if (Array.isArray(entry[obj])) {
						out += "switch ($_GET['" + obj + "']) {\n";
						out += entry[obj].reduce((acc, id) => `${acc}	case ${id}:\n`,"");
						ifs++;
					} else {
						out += `if ($_GET['${obj}'] == ${entry[obj]}) {\n`;
						ifs++;
					}
				}
				out += `header("Location: /${url}", TRUE, 301);\nexit();\n`;
				out += '}'.repeat(ifs);
				out += "\n\n";
			});
			writeFile(key, `<?php
