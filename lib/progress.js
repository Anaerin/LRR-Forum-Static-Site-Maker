...
class Progress {
	...
	completeTask(taskName) {
		if (this.tasks.has(taskName) && this.tasks.get(taskName).whenStarted) {
			...
			console.log(`${taskName} Finished, took ${this.calculateDuration((Date.now() - taskValues.whenStarted)/1000)}`);
	...
	updateTask(taskName, progress, total) {
		if (this.tasks.has(taskName) && this.tasks.get(taskName).whenStarted) {
			...
			this.tasks.set(taskName, taskValues);
			console.log(`${taskName} Progress: ${progress}/${total}${taskValues.eta}`);
	...
	writeUpdate() {
		...
		let out = [];
		if (!this.finished)	this.tasks.forEach((value, key) => {
			out.push(value);
		});
		try {
			fs.writeFileSync(resolvedPath, JSON.stringify(out), { flush: true, encoding: "utf8" });
		} catch(e) {
			console.log(`Couldn't write to progress file, filename: ${resolvedPath}, Data: "${JSON.stringify(out)}": ${e}`);
		}
	...
	calculateDuration(amount) {
		let timeParts = amount;
		let out = "";
		if (timeParts > 9) out = `${this.padNumber(timeParts % 60,2)} seconds`;
		else out = `${timeParts} seconds`;
	...
	estimateTTC(startTime, progress, total) {
		...
		return this.calculateDuration(timeParts) + " remaining";
	}
